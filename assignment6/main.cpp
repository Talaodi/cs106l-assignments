...
public:
  ...
  {
    auto it = std::ranges::find_if(courses, [&course_title](const auto& course) {
...
main(int argc, char* argv[])
{
  ...
  if (argc == 2) {
    ...
    ********************************************************/
    ...
    std::string output = course.and_then([](const auto& course) {
      std::ostringstream out;
      out << "Found course: " << course->title << "," << course->number_of_units << "," << course->quarter;
      return std::optional { out.str() };
